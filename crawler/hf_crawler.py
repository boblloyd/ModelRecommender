...
def _base_model_matches(item: dict, base_model: str) -> bool:
    """
    Return True if the model is compatible with the target base model.
    Unknown base models pass through unfiltered.

    HuggingFace auto-generates base_model:<repo_id> tags from adapter_config.json,
    so many LoRAs have the tag but no cardData.base_model entry.  Check tags first.
    """
    expected_ids = BASE_MODEL_HF_REPO_IDS.get(base_model, [])
    if not expected_ids:
        return True

    lowered = [e.lower() for e in expected_ids]

    # Primary: base_model:<repo> tag generated by HF (present even without card YAML)
    for tag in item.get("tags") or []:
        if tag.startswith("base_model:"):
            repo = tag[len("base_model:"):].lower()
            if any(e in repo or repo in e for e in lowered):
                return True

    # Fallback: explicit cardData.base_model field
    ...
    return any(e in c for e in lowered for c in candidates)
...
async def full_crawl(base_model: str, pool: asyncpg.Pool) -> int:
    ...
    async with httpx.AsyncClient() as client:
        ...
        while True:
            ...
            total += inserted
            log.info("  → %d fetched, %d upserted (total: %d)", len(items), inserted, total)
